(function () {
  ...
  function seedState() {
    ...
    const events = [
      makeEvent(today, 0, 9, 0, 9, 30, {
        id: 'evt_ops_standup',
        title: 'Ops standup',
        type: 'meeting',
        notes: 'Review blockers, confirm deployments, and assign the afternoon follow-ups.',
        attachments: [
          { label: 'Agenda doc', href: '#' },
          { label: 'Ops board', href: '#' }
        ],
        location: 'War room'
      }),
      makeEvent(today, 0, 10, 0, 12, 0, {
        id: 'evt_focus_today',
        title: 'Deep work: calendar orchestration',
        type: 'focus',
        notes: 'Protect this block for planning logic, edge cases, and automation behavior.'
      }),
      makeEvent(today, 0, 12, 30, 13, 0, {
        id: 'evt_lunch_reset',
        title: 'Lunch away from desk',
        type: 'reminder',
        notes: 'Step away before the afternoon meetings.'
      }),
      makeEvent(today, 0, 15, 30, 16, 0, {
        id: 'evt_budget_ping',
        title: 'Budget follow-up ping',
        type: 'reminder',
        notes: 'Send a quick note to finance before end of day.'
      }),
      makeEvent(today, -1, 15, 0, 15, 20, {
        id: 'evt_missed_invoice',
        title: 'Send contractor invoice',
        type: 'reminder',
        notes: 'Missed yesterday. It should be rescheduled this morning.',
        missed: true
      }),
      makeEvent(today, 1, 8, 15, 8, 45, {
        id: 'evt_followup_workflow',
        title: 'Follow-up workflow',
        type: 'automation',
        notes: 'Recurring routine that creates prep reminders and recap tasks for client calls.',
        recurring: 'Every Tuesday',
        attachments: [{ label: 'Automation log', href: '#' }]
      }),
      makeEvent(today, 1, 14, 0, 15, 0, {
        id: 'evt_client_call',
        title: 'Call client - Northwind',
        type: 'meeting',
        notes: 'Renewal conversation with pricing options, rollout timing, and onboarding capacity.',
        attachments: [
          { label: 'Call brief', href: '#' },
          { label: 'Northwind contract', href: '#' }
        ],
        location: 'Meet link'
      }),
      makeEvent(today, 1, 13, 15, 13, 45, {
        id: 'evt_prep_brief',
        title: 'Prep brief for Northwind',
        type: 'task',
        notes: 'Generated by the routine ahead of the client call.'
      }),
      makeEvent(today, 1, 15, 15, 15, 45, {
        id: 'evt_recap_task',
        title: 'Recap notes + follow-up',
        type: 'task',
        notes: 'Post-call summary and next steps.'
      }),
      makeEvent(today, 2, 9, 0, 12, 0, {
        id: 'evt_focus_tomorrow',
        title: 'Deep work: AI scheduling logic',
        type: 'focus',
        notes: 'Protected strategy block for shipping the parser and event actions.'
      }),
      makeEvent(today, 2, 10, 30, 11, 30, {
        id: 'evt_strategy_review',
        title: 'Strategy review',
        type: 'meeting',
        notes: 'This overlaps the focus block and is a good candidate for a slight move.',
        attachments: [{ label: 'Planning deck', href: '#' }],
        location: 'Conference room'
      }),
      makeEvent(today, 2, 16, 0, 16, 30, {
        id: 'evt_payroll_cutoff',
        title: 'Payroll cutoff',
        type: 'deadline',
        notes: 'Approve team hours before end of day.'
      }),
      makeEvent(today, 3, 11, 0, 11, 45, {
        id: 'evt_partner_sync',
        title: 'Partner sync',
        type: 'meeting',
        notes: 'Status check on deployment and migration timing.',
        attachments: [{ label: 'Migration checklist', href: '#' }]
      }),
      makeEvent(today, 3, 16, 0, 16, 30, {
        id: 'evt_proposal_deadline',
        title: 'Proposal deadline',
        type: 'deadline',
        notes: 'Final packet has to be sent before the close of business.'
      }),
      makeEvent(today, 4, 9, 0, 9, 20, {
        id: 'evt_tax_reminder',
        title: 'File GST reminder',
        type: 'reminder',
        notes: 'Prepare the checklist before Friday afternoon.'
      }),
      makeEvent(today, 4, 17, 0, 17, 30, {
        id: 'evt_weekly_wrap',
        title: 'Weekly wrap-up workflow',
        type: 'automation',
        notes: 'Builds the Friday summary, captures wins, and schedules Monday prep.',
        recurring: 'Weekly'
      })
    ].sort(compareEvents);

    const tasks = [
      makeTask(today, 0, 16, 30, {
        id: 'tsk_followup',
        title: 'Draft investor follow-up',
        duration: 45,
        priority: 'high',
        notes: 'Capture next steps while the morning context is fresh.',
        tags: ['follow-up', 'sales']
      }),
      makeTask(today, 1, 11, 0, {
        id: 'tsk_sync_spec',
        title: 'Write Google sync spec',
        duration: 90,
        priority: 'urgent',
        notes: 'This needs a full focus block before the proposal deadline.',
        tags: ['calendar', 'backend', 'focus']
      }),
      makeTask(today, 2, 14, 0, {
        id: 'tsk_move_review',
        title: 'Re-slot strategy review deck',
        duration: 30,
        priority: 'normal',
        notes: 'Tune the talking points if the meeting moves.',
        tags: ['meeting']
      }),
      makeTask(today, -1, 10, 0, {
        id: 'tsk_overdue_notes',
        title: 'Send studio notes to Aly',
        duration: 30,
        priority: 'high',
        notes: 'Overdue from yesterday and still not scheduled.',
        tags: ['overdue']
      }),
      makeTask(today, 3, 9, 30, {
        id: 'tsk_briefing',
        title: 'Record async briefing',
        duration: 60,
        priority: 'normal',
        notes: 'Useful filler for the next free window.',
        tags: ['content']
      })
    ].sort(compareTasks);
